...
| 4-if_9_say_hi | Displays `Best School` 10 times using a while loop, but on the 9th iteration displays `Best School` and `Hi` on a new line |
| 5-4_bad_luck_8_is_your_chance | Loops 10 times using a while loop, on the 4th iteration displays `bad luck`, on the 8th iteration displays `good luck`, and on all other iterations displays `Best School` |
...
| 9-to_file_or_not_to_file | Gives the user information about the `school` file |
